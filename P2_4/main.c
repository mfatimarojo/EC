...
static struct RLstat RL0 = { .moving = 0, .speed = 5, .direction = 0,
		.position = 0, };

static struct RLstat RL1 = { .moving = 0, .speed = 5, .direction = 0,
		.position = 3, };
...
void timer0_ISR(void) {
	if (RL0.direction) {
		if (RL0.position == 5)
			RL0.position = 0; //Rotatorio
		else
			RL0.position++;
	} else {
		if (RL0.position == 0)
			RL0.position = 5; //Rotatorio
		else
			RL0.position--;
	}

	//D8Led_segment(RL0.position);
	D8Led_2segments(RL0.position, RL1.position);
...
void timer1_ISR(void) {
	if (RL1.direction) {
		if (RL1.position == 5)
			RL1.position = 0; //Rotatorio
		else
			RL1.position++;
	} else {
		if (RL1.position == 0)
			RL1.position = 5; //Rotatorio
		else
			RL1.position--;
	}

	//D8Led_segment(RL1.position);
	D8Led_2segments(RL0.position, RL1.position);
...
void keyboard_ISR(void) {
	...
	if (key != -1) {
		...
		case 4:
			...
			tmr_set_count(TIMER1, 362500, 31250);
			break;
		case 5:
			tmr_set_divider(TIMER1, D1_8);
			tmr_set_count(TIMER1, 31250, 15625);
			break;
		case 6:
			tmr_set_divider(TIMER1, D1_8);
			tmr_set_count(TIMER1, 15625, 7812);
			break;
		case 7:
...
int setup(void) {
	...
	pISR_TIMER1 = (unsigned) timer1_ISR;	//COMPLETAR: registrar la RTI del timer
