...
int kb_scan(void)
{
	...
	for (i = 0; (i < 4) && (value == -1); i++) {
		...
		if(( temp & KEY_VALUE_MASK) != KEY_VALUE_MASK) {
			//COMPLETAR: 
			if (!(temp & 0x1))		// si está el bit 1 a 0 la tecla es la map[i][3]
				value = map[i][3];
			if (!(temp & 0x2))		// si está el bit 2 a 0 la tecla es la map[i][2]
				value = map[i][2];
			if (!(temp & 0x4))		// si está el bit 3 a 0 la tecla es la map[i][1]
				value = map[i][1];
			if (!(temp & 0x8))		// si está el bit 4 a 0 la tecla es la map[i][0]
				value = map[i][0];
		}
	}
